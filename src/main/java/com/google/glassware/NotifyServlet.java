    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (notification.getCollection().equals("locations")) {
            ...
            if (notification.getUserActions().contains(new UserAction().setType("SHARE"))
                ...
                final String tlenImageUrlString = ImageProcessingUtil.tlenifyImage(stream);
                LOG.info("tlenned image : " + tlenImageUrlString);
                URL tlenImageUrl = new URL(tlenImageUrlString);

                // insert tlenned image
                MirrorClient.insertTimelineItem(credential, timelineItem, "image/jpeg", tlenImageUrl.openStream());
